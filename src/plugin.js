...
function loadForecast(latLon) {
	...
	windyHttp.get(windyUrls.getPointForecast(model, Object.assign({step: 1}, getLatLon(latLon)), 'detail')).then(forecast => {
...
function getTooltip(latLon) {
	...
	const tooltips = localSites.map(site => {
		return '<b style="font-size: 1.25em;' + (site.name.length >= 20 ? 'text-overflow: ellipsis; max-width: 180px; display: inline-block; overflow: hidden; vertical-align: text-bottom;" title="' + html(site.name) : '') + '"><a' + getLaunchAttrs(site)
			...
			+ ' <a href="https://mapy.cz/turisticka?source=coor&id=' + site.longitude + ',' + site.latitude + '" target="_blank"><img src="https://mapy.cz/img/favicon/favicon.ico" width="12" height="12" alt="" title="' + translate('takeoff', 'startovačka') + '" style="vertical-align: middle;"></a>'
			+ getLaunchExtra(site)
		;
	});
	...
	extra.push('<span title="' + translate('lower from intersections of dry adiabat with temperature and isogram', 'nižší z průsečíků suché adiabaty s teplotou a izogramou') + '">'
		+ (cloudBase ? translate('Cloud base', 'Základny') : translate('Possible climb', 'Dostupy')) + '</span>:'
